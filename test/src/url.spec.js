...
describe('Url', () => {
  describe('getUrlParam', () => {
    it('should return null on undefined urls without default value', () => {
      const param = getUrlParam('__dactylographsy__', null, 'http://localhost');
      const param2 = getUrlParam('__dactylographsy__', null, 'http://localhost?foo=bar');
    ...
    it('should return the default value on urls with default value', () => {
      const param = getUrlParam('__dactylographsy__', 'bar3', 'http://localhost?foo=bar&foo=bar2');

      param.should.equal('bar3');
    });

    it('should return the value on urls without default value', () => {
      const param = getUrlParam('foo', null, 'http://localhost?foo=bar');

      param.should.equal('bar');
    });

    it('should return the last value on urls without default value', () => {
      const param = getUrlParam('foo', null, 'http://localhost?foo=bar&foo=bar2');

      param.should.equal('bar2');
    });

    it('should return the value on urls with default value', () => {
      const param = getUrlParam('foo', 'bar', 'http://localhost?foo=bar&foo=bar2');

      param.should.equal('bar2');
    });

    it('should maintain types of values within the url', () => {
      const param1 = getUrlParam('foo', null, 'http://localhost?foo=true');
      const param2 = getUrlParam('bar', null, 'http://localhost?bar=3');
    ...
    it('should return encoded version of strings', () => {
      const param = getUrlParam('foo', null, 'http://localhost?foo=<script>alert(document.cookie)</script>');
